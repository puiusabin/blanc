...
const { db, threads, emails } = require("@blanc/database");
 ...
 * @param {Object} dbClient - Drizzle database client instance
 * @returns {Promise<String|null>} Thread UUID (existing or null for new thread)
 */
async function computeThreadId(headers, userId, dbClient) {
  ...
  if (headers.references && headers.references.length > 0) {
    ...
    const [emailMatch] = await dbClient
      .select({ threadId: emails.threadId })
      .from(emails)
      .where(eq(emails.userId, userId))
      .where(eq(emails.messageId, rootMessageId))
      .where(eq(emails.threadId, null))
      .limit(1);

    if (emailMatch && emailMatch.threadId) {
      return emailMatch.threadId;
    }
  }
  ...
  if (headers.inReplyTo) {
    const [parentEmail] = await dbClient
      .select({ threadId: emails.threadId })
      .from(emails)
      .where(eq(emails.userId, userId))
      .where(eq(emails.messageId, headers.inReplyTo))
      .limit(1);
